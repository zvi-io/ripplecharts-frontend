...
function MarketsCtrl($scope, $state, $location, gateways) {
        ...
        emit: function (type, data) {
            if (type === 'spread') {
                document.title = data.bid + '/' +
                    data.ask + ' ' +
                    translateCoin($scope.base.currency) + '/' +
                    translateCoin($scope.counter.currency)
            }
        }
    })
    ...
    function handleTransition(mode) {
        ...
        $('#base_currency > div > a > label').text(translateCoin($('#base_currency > div > a > label').text()));
        $('#counter_currency > div > a > label').text(translateCoin($('#counter_currency > div > a > label').text()));
    ...
    function loadDropdowns() {
        ...
        $('#base_currency > div > a > label').text(translateCoin($('#base_currency > div > a > label').text()));
        $('#counter_currency > div > a > label').text(translateCoin($('#counter_currency > div > a > label').text()));
        $('#base_currency > ul > li:nth-child(1) > a > label').text(translateCoin($('#base_currency > ul > li:nth-child(1) > a > label').text()));
        $('#counter_currency > ul > li:nth-child(1) > a > label').text(translateCoin($('#counter_currency > ul > li:nth-child(1) > a > label').text()));
    }
    ...
    function setParams() {
        if ($state.params.base && $state.params.counter) {
            ...
            $scope.counter = store.session.get('counter') ||
                store.get('counter') ||
                Options.counter ||
                {currency: 'XSD', issuer: 'rMJSrGBUCTYnJN9kPNdgEm2hAfuC6bfPi8'}
        }

        if ($scope.base.currency === 'XRP' &&
            $scope.counter.currency === 'XRP') {
            $scope.counter = Options.counter ||
                {currency: 'XIM', issuer: 'rMJSrGBUCTYnJN9kPNdgEm2hAfuC6bfPi8'}
        }
